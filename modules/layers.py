...
__all__ = ['forward_hook', 'Clone', 'Add', 'Cat', 'ReLU', 'Tanh', 'Dropout', 'BatchNorm2d', 'Linear', 'MaxPool2d',
...
class Linear(nn.Linear, RelProp):
    def relprop(self, R, alpha = 1):
        beta = alpha - 1
        pw = torch.clamp(self.weight, min=0).requires_grad_(True)
        nw = torch.clamp(self.weight, max=0).requires_grad_(True)
        px = torch.clamp(self.X, min=0).requires_grad_(True)
        nx = torch.clamp(self.X, max=0).requires_grad_(True)
